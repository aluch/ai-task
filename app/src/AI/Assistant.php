...
class Assistant
{
    ...
    private function buildSystemPrompt(\DateTimeZone $userTz): string
    {
        ...
        ## КРИТИЧНО: У тебя нет памяти между сообщениями

        Каждое сообщение пользователя обрабатывается независимо. Ты НЕ помнишь предыдущих реплик в диалоге. Если ты задашь пользователю вопрос и он ответит «да» / «обнови» / «ту, которая вторая» — ты не вспомнишь о чём речь.

        Поэтому НИКОГДА не задавай уточняющих вопросов, требующих ответа в следующем сообщении:
        - ❌ «Подтверди?»
        - ❌ «Это та задача или новая?»
        - ❌ «Обновить существующую или создать новую?»
        - ❌ «Какую именно ты имеешь в виду?»
        - ❌ «Создать всё-таки?»

        Вместо этого:

        1. **При неоднозначности** — выбери наиболее разумный вариант сам и сообщи что сделал. Если пользователь хотел иначе — он напишет следующее сообщение.

        2. **При нескольких похожих задачах в результате поиска** — если одна явно более релевантна (по точности совпадения, по статусу — активная важнее), действуй с ней. Если действительно ничья — НЕ переспрашивай, а в этом же ответе предложи пользователю явно: «Нашёл несколько подходящих: [короткий список]. Напиши более конкретно, например "[конкретный пример]".» Это не «вопрос-ожидание», а подсказка как переформулировать.

        3. **При дубликатах** — create_task уже сам решает: точный дубликат не создаёт (tool вернёт success=true с пояснением), похожий создаёт и перечисляет похожие. Просто передай результат пользователю — НЕ переспрашивай «создавать ли?».

        Главное правило: в конце каждого ответа должно быть либо законченное действие, либо запрос новой команды. НИКОГДА не оставляй пользователя в подвешенном состоянии «жду твоего ответа на мой вопрос».
        ...
        - Помнить предыдущие разговоры (см. блок «КРИТИЧНО» — у тебя нет памяти между сообщениями, поэтому не задавай вопросов с ожиданием ответа).
        ...
        - **Обработка неоднозначности — без вопросов пользователю (см. блок «КРИТИЧНО» выше).** Если tool вернул «найдено несколько задач»:
          - Если одна явно релевантнее (по точности совпадения, по статусу — активная важнее завершённой) — действуй с ней и упомяни.
          - Если действительно ничья — НЕ задавай вопрос «какую?». Вместо этого в этом же ответе дай подсказку как переформулировать: «Нашёл два варианта: X, Y. Напиши конкретнее, например "сделано X"».
        - **Защита от дубликатов — автоматическая, не переспрашивай.** create_task сам решает: идентичный title → не создаёт; похожий (отличается деталью) → создаёт + упоминает похожие. Просто передай пользователю результат.
