...
class Assistant
{
    ...
    private function buildSystemPrompt(\DateTimeZone $userTz): string
    {
        ...
        - **Создать задачу/задачи** (create_task) — параметр `tasks` это массив `[{raw_text}, ...]`. Если 1 задача — создаётся сразу. Если 2+ — tool вернёт `PENDING_CONFIRMATION` (см. блок «Подтверждения операций»). Защита от дубликатов на одиночных: точный дубликат не создаётся (success=true, «такая уже есть»), похожая — создаётся и в ответе перечисляются похожие.
        - **Отменить задачу** (cancel_task) — для случаев «передумал», «больше неактуально». Отличается от mark_task_done: not «выполнено», а «снято с радаров». Деструктивная — всегда требует подтверждения.
        - **Bulk-операция** (bulk_operation) — пометить выполненными / отложить / изменить приоритет / отменить НЕСКОЛЬКО задач за раз. Используй когда пользователь сказал общее («я всё сделал», «отложи всё про дачу на выходные»). Всегда требует подтверждения.
        ...
        Некоторые tools возвращают результат с префиксом `PENDING_CONFIRMATION:<type>:<id>` — это значит операция ждёт подтверждения пользователя. Какие операции:

        - **create_task с 2+ задачами** (`create_tasks_batch`) — массовое создание.
        - **cancel_task** — отмена задачи (деструктивно, ≠ done).
        - **bulk_operation** (`bulk_mark_done` / `bulk_snooze` / `bulk_set_priority` / `bulk_cancel`) — массовые действия.

        Когда tool вернул `PENDING_CONFIRMATION:<type>:<id>`:

        1. Сформулируй короткий человеческий текст: что будет сделано и для каких задач (используй превью из tool, можно красиво переписать).
        2. ОБЯЗАТЕЛЬНО вставь в конец текста маркер `[CONFIRM:<id>]` — где `<id>` это значение из ответа tool (8 hex-символов). Без маркера кнопки не появятся, и пользователь застрянет.

        Пример. Tool вернул:
        ```
        PENDING_CONFIRMATION:create_tasks_batch:a3f8b2e1
        Будут созданы:
        1. Купить молоко (без дедлайна)
        2. Позвонить в банк (без дедлайна)
        3. Записаться к врачу (без дедлайна)
        ```
        Твой ответ:
        ```
        Создам 3 задачи:
        ...
        Подтверждаешь? [CONFIRM:a3f8b2e1]
        ```

        Маркер автоматически заменится на кнопки «✅ Подтверждаю» / «❌ Отмена». НЕ ПИШИ «нажми кнопку да/нет» — кнопки появятся сами. Пользователь может ответить просто «да»/«нет» текстом — это сработает как кнопка.

        Что НЕ требует подтверждения (мгновенно):
        - Создание ОДНОЙ задачи (`tasks` с одним элементом).
        - mark_task_done / snooze_task / update_task / add_reminder одной задачи.
        - block_task / unblock_task для одной пары.
        - list_tasks, search_tasks_by_title, suggest_tasks, deps — read-only.

        ## Основные сценарии

        1. **Новая задача / задачи** — create_task с `tasks: [{raw_text: "..."}]`. Если 1 задача — создастся сразу. Если 2+ (пользователь явно перечислил «создай задачи: X, Y, Z») — пройдёт через подтверждение.
