    ...
    private function buildSystemPrompt(
        ...
        8. Reminder_interval_minutes — как часто пинать пользователя о незавершённой
           ...
           - Пользователь ЯВНО просит: «напоминай каждый день», «пинай каждые
             2 часа», «не давай забыть». Бери интервал из запроса ровно как
             указано — не заменяй на «разумное» значение.
           ...
           Минимальный интервал — 60 минут. Если пользователь явно просит
           меньше — клампай к 60 (ставь 60), НЕ подменяй на типовое значение
           вроде 180/360. Если пользователь не указал интервал — используй
           дефолты по priority выше.

           Примеры:
           - «напоминай каждые 5 минут»  → 60 (кламп, в notes упомяни минимум)
           - «напоминай каждые 15 минут» → 60 (кламп)
           - «напоминай каждые 30 минут» → 60 (кламп)
           - «напоминай каждый час»      → 60
           - «напоминай каждые 2 часа»   → 120 (как просил, не 360)
           - «напоминай каждые 3 часа»   → 180
           - «напоминай каждые 6 часов»  → 360
           - «напоминай каждый день»     → 1440
