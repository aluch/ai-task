...
Сущности:
- `App\Entity\Subscription` — план + статус + период (`currentPeriodStart`/`End`), `trialEndsAt` для триала, `externalSubscriptionId` для будущей привязки к ЮKassa. В S2 добавлены `notification_3d_sent_at` / `_1d_sent_at` / `_expired_sent_at` для дедупа триальных уведомлений. В S3 — `convertedFromTrialAt` (момент перехода триал→Pro, заполняется в `activatePro` если предыдущий статус был trialing; используется метрикой конверсии в `/admin stats`).
...
- `App\Service\Subscription\SubscriptionService` — `startTrial` (идемпотентен, защита от абуза), `activatePro` (фиксирует `convertedFromTrialAt` если был триал), `cancel` (доступ до `currentPeriodEnd`), `expire`, `getCurrentPlan`, `getActiveSubscription`, `isRefundEligible`. **S3:** `forceStartTrial` / `forceActivatePro` — админские force-методы для `/admin grant_*`: гасят существующую подписку в expired и создают новую (не идемпотентны).
...
- `App\Telegram\UI\SoftBlockMessageBuilder` — текст + клавиатура для soft block при превышении лимита. Возвращает `{text, keyboard}` в формате Bot API. Кнопка `upgrade:info` ведёт на `/upgrade`.
- `App\Telegram\UI\UpgradeMessageBuilder` (S3) — тексты `/upgrade` для четырёх статусов (admin / active Pro / trialing / free).
- `App\Telegram\UI\SubscriptionMessageBuilder` (S3) — тексты `/subscription` для admin / active / trial / cancelled / free + confirm-экран отмены.
