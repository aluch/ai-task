...
## Связанные задачи (отложено до Этапа 4)

Задачи могут быть связаны по месту, маршруту или логически (например, «зайти в WB» и «зайти в АЛСО» находятся рядом, «купить подарок» и «упаковать подарок» — последовательные). Два подхода к реализации:

1. **Явные связи в модели** — `Task.relatedTasks: ManyToMany(self)`. Полезно если парсер умеет определять связь в момент создания. Сложность: парсер обрабатывает каждое сообщение изолированно и не видит другие задачи пользователя, поэтому связи можно выстроить только вторым проходом.

2. **Умная группировка при подборе (приоритетный подход)** — на этапе `/free` AI получает весь список открытых задач пользователя и сам группирует подходящие по месту, маршруту или логике. Не требует изменения модели данных. Работает для задач, созданных в разное время и из разных источников.

**Решение**: начинаем с подхода 2 (Этап 4, реализация `/free`). Подход 1 добавим позже если понадобится явная привязка — например, для визуализации графа задач или для явного «сделай А перед Б».
