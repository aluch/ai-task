...
Связь `Task ↔ Task` (зависимости) — many-to-many self-ref через `task_dependencies (blocked_task_id, blocker_task_id)`, PK на паре, оба FK с `ON DELETE CASCADE`.
...
Задача может быть заблокирована одной или несколькими другими задачами — пока блокеры не выполнены или отменены, заблокированная задача не может быть взята в работу.

### Модель

`Task.blockedBy: ManyToMany(self)` — join-таблица `task_dependencies (blocked_task_id, blocker_task_id)`. Обратная сторона — `Task.blocking` (inversed). Оба FK с `ON DELETE CASCADE`.

### Правила разблокировки

Задача считается **заблокированной** (`isBlocked() === true`) если у неё есть хотя бы один блокер в статусе, отличном от `DONE` и `CANCELLED`. Методы:

- `getActiveBlockers(): Task[]` — незавершённые блокеры
- `isBlocked(): bool` — есть ли активные блокеры
- `getBlockedTasks(): Collection` — задачи, которые блокирует текущая (обратная сторона)

Статусы `DONE` и `CANCELLED` оба разблокируют — это позволяет «обойти» зависимость отменой блокирующей задачи.

### Проверка циклов

`App\Service\DependencyValidator::validateNoCycle(blocked, blocker)` — DFS от blocker по его `blockedBy`. Если достигнем blocked — бросает `CyclicDependencyException`. Самоблокировка (`blocked === blocker`) проверяется отдельно через `\LogicException` в `Task::addBlocker()`.

### Telegram-команды

- `/block <task_id> <blocker_id>` — создать зависимость
- `/unblock <task_id> <blocker_id>` — убрать зависимость
- `/deps <task_id>` — показать зависимости в обе стороны

### Отображение

- `/list`: заблокированные задачи показываются в конце списка с пометкой ⛔
- `/done`: при выполнении задачи, если разблокировались другие задачи — выводится уведомление

### Отложено

- **AI-парсинг зависимостей** из текста («сначала X, потом Y»). Сейчас TaskParser не видит другие задачи пользователя при создании.
- **Умная группировка при подборе** — на этапе `/free` AI получит весь список задач и сам определит связи по месту/маршруту/логике. Не требует явных связей в модели.
