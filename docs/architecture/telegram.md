...
| `/block <task> <blocker>` | Задача task заблокирована blocker'ом. Проверяет циклы и самоблокировку |
| `/unblock <task> <blocker>` | Убрать зависимость |
| `/deps <id>` | Показать зависимости задачи в обе стороны (blockedBy + blocking) |
