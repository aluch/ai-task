...
- S5 — auto-rebill через сохранённый токен + webhook ❌ ОТКАЧЕНО (см. ниже)
- S5 (replacement) — manual renewal через /upgrade + 3d/1d/expired notifications ✅
- S6 — refund через ЮKassa Refund API + admin stats по refund'ам

## Почему нет auto-rebill (на 2026-05)

ЮKassa поддерживает recurring через `save_payment_method=true`, но **Telegram Payments не передаёт** этот параметр корректно. После платежа ЮKassa возвращает `payment_method.saved=false`, что означает «карту нельзя использовать повторно».

Это известная нестыковка. Без сохранённой карты технически невозможно списать второй раз без участия пользователя.

### Текущая модель

Подписка продляется **только вручную** через `/upgrade`. За 3 дня и 1 день до истечения — уведомления (`RenewalNotifier`). В момент истечения — переход на Free + уведомление. Подробное руководство по UX — `docs/subscriptions-recurring.md`.

Это легитимная и распространённая модель: Patreon, многие Telegram-боты, некоторые SaaS работают так же.

### Что осталось как dead code

Откатывали аккуратно, чтобы быстро вернуться когда платформа поддержит. Не удалены:

- БД-поля: `subscriptions.saved_payment_method_id`, `auto_rebill_enabled`, `notification_24h_before_rebill_sent_at`, `rebill_failed_attempts`. Все с `DEAD CODE` docblock'ами в `App\Entity\Subscription`.
- Таблица `recurring_attempts` + сущность `RecurringAttempt`.
- `YooKassaApiClient::createRecurringPayment` / `removePaymentMethod`. `getPayment` тоже остался, но `PaymentProcessor` его больше не дёргает.
- `RebillWebhookProcessor` + `YooKassaWebhookController` (POST `/api/yookassa/webhook`) + `YooKassaIpAllowlist`. Endpoint пригодится для refund'ов в S6.
- `RebillScheduler` 4 «старых» метода (notifyUpcomingCharges, initiateCharges, retryFailedAttempts, expirePastDueSubscriptions) — помечены DEAD CODE. `run()` теперь дёргает `RenewalNotifier`.
- `InvoicePayloadBuilder::buildProviderDataForRecurring`.

### Когда вернёмся к recurring

Когда **что-то** из этого случится:

- Telegram Payments начнёт передавать `save_payment_method` в ЮKassa.
- Перейдём на ЮKassa Checkout (отдельная платёжная страница вместо встроенной Telegram).
- Перейдём на Telegram Stars (нативная подписочная валюта Telegram).

Инфраструктура (RecurringAttempt, YooKassaApiClient.createRecurringPayment, webhook endpoint) — **готова**, останется только включить логику и обновить UI.
