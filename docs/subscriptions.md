...
- `upgrade:pay` — заглушка «Оплата через Telegram Payments скоро появится» (S4 заменит на ЮKassa-инвойс). Для админа — админский текст.
...
| S5 | Auto-rebill — продление через `external_subscription_id` ЮKassa, email-уведомления |
